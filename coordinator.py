...
"""Poll ZoneMinder’s Events table for bins, rolling counts, last start, and active."""
...
def __init__(self, hass, config):
    ...
    def fetch():
        ...
        try:
            ...
            for row in cursor.fetchall():
                mid = row["MonitorId"]
                data[mid]["rolling_count"] = int(row["rolling_count"])
                data[mid]["last_start"] = row["last_start"].isoformat()

            # 4) Active count (in-progress recordings)
            cursor.execute("""
                SELECT
                  MonitorId,
            ...
            for row in cursor.fetchall():
                mid = row["MonitorId"]
                data[mid]["active_count"] = int(row["active_count"])
